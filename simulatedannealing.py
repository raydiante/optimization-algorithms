...
def SimulatedAnnealing(matriz,card):
	s=[]
	sl=[] 
	i=0
	p=50
	...
	melhor=calculaFMA(s)
	while(k<(p*100)):
		sl= vizinhanca(s,card,2)
		k+=1

		if i>=p and calculaFMA(sl) > calculaFMA(s):
			s=sl
			if calculaFMA(s)<melhor:
				melhor= calculaFMA(s)
		else:
			if calculaFMA(sl)<calculaFMA(s):
				s=sl
			i+=1
	print melhor
...
def Main():
	...
	SimulatedAnnealing(matriz,card)
